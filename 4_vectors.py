    ...
    for user_post in comment_list:
        if len(user_post) > 1:
            if any(x in str(user_post[1]) for x in matches_1):
                ...
                team = 3
                sentiment_list.append([team, user_post[-3]])
            elif any(x in str(user_post[1]) for x in matches_4):
                team = 4
                sentiment_list.append([team, user_post[-3]])
...
def likelyhoods(raw_sentiment): # calculates chance of each word occuring in class x
    ...
    post_count_4 = dict.fromkeys(word_atlas, 0)
    ...
    post_count_4_old = post_count_4.copy()
    for word in word_atlas:
        post_count_1[word] = (post_count_1_old[word] + +1) / (
                    4 + post_count_1_old[word] + post_count_2_old[word] + post_count_3_old[word] + post_count_4_old[
                word])
        post_count_2[word] = (post_count_2_old[word] + +1) / (
                    4 + post_count_1_old[word] + post_count_2_old[word] + post_count_3_old[word] + post_count_4_old[
                word])
        post_count_3[word] = (post_count_3_old[word] + 1) / (
                    4 + post_count_1_old[word] + post_count_2_old[word] + post_count_3_old[word] + post_count_4_old[
                word])
        post_count_4[word] = (post_count_4_old[word] + 1) / (
                    4 + post_count_1_old[word] + post_count_2_old[word] + post_count_3_old[word] + post_count_4_old[
                word])
 ...
 #   post_count_3 = {key: (value) / count3 for key, value in post_count_3.items()}
    ...
    priors_from_frequency.append([count1/total, count2/total, count3/total, count4/total])
    return post_count_1, post_count_2, post_count_3, post_count_4


probability_dct = likelyhoods(user_sentiments)
preset_priors = [priors_from_frequency[0][0], priors_from_frequency[0][1], priors_from_frequency[0][2], priors_from_frequency[0][3]]


def posteriors(priors_input, list1): # calculates posteriors
    c1 = 0
    post_1 = [a * b for a, b in zip(priors_input, list1)]
    if post_1[0] + post_1[1] + post_1[2] + post_1[3] > 0:
        post_1_1 = (post_1[0]) / (post_1[0] + post_1[1] + post_1[2] + post_1[3])
        post_1_2 = (post_1[1]) / (post_1[0] + post_1[1] + post_1[2] + post_1[3])
        post_1_3 = (post_1[2]) / (post_1[0] + post_1[1] + post_1[2] + post_1[3])
        post_1_4 = (post_1[3]) / (post_1[0] + post_1[1] + post_1[2] + post_1[3])
        c1 += 1
        #print("success", c1)
    else:
        c1 -= 1
        ...
        post_1_4 = priors_from_frequency[0][3]
    return post_1_1, post_1_2, post_1_3, post_1_4
...
def find_word_probs(comments_inpt):
    ...
    for commt in comments_inpt:
        if len(commt) > 0:
            ...
            for wrd in word_atlas:
                if wrd in commt[-3]:
                    addition.append([wrd, probability_dct[0][wrd], probability_dct[1][wrd], probability_dct[2][wrd], probability_dct[3][wrd]])
    ...
    for sentence_probs in found_probs:
        ...
        if len(sentence_probs2) > 0:
            for pair in range(len(sentence_probs2)-1):
                if counter == 0:
                    first_pair = posteriors(preset_priors, sentence_probs2[pair][-4:])
                first_pair = posteriors(first_pair, sentence_probs2[pair+1][-4:])
                counter += 1
...
def check_accuracy(prediction_scores):
    ...
    for comment in prediction_scores:
        print(comment[0])
        # make the prediction
        if comment[0][0] == max(comment[0]):
            prediction = "ferrari"
        elif comment[0][1] == max(comment[0]):
            prediction = "red-bull"
        elif comment[0][2] == max(comment[0]):
            prediction = "mercedes"
        elif comment[0][3] == max(comment[0]):
            prediction = "other"
        ...
        if prediction == answer:
            right_answers += 1
        elif answer == "red-bull" or answer == "ferrari" or answer == "mercedes" or answer == "other":
            wrong_answers += 1
