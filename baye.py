...
def find_word_probs(comments_inpt):
    ...
    for commt in comments_inpt:
        ...
        comments_probs.append(addition)
...
def classifier(found_probs):
    for sentence_probs in found_probs:
        ...
        if len(sentence_probs) > 1:
            for pair in range(len(sentence_probs)-1):
                if counter == 0:
                    first_pair = sentence_probs[pair][-2:]
                first_pair = posteriors(first_pair, sentence_probs[pair+1][-2:])
                counter += 1
