...
upper_limit_training = 3000
lower_limit_testing = 3000
upper_limit_testing = 4900
the_path = "pol4.csv"
...
def count_words(list_with_text, text_position): #text_position ist the place the text body has in the list
    ...
    count_dict = dict.fromkeys(count_list)
    print(count_dict)
    return count_dict
    print("hey")
...
def likelyhoods(raw_sentiment, n): # calculates chance of each word occuring in class x n is number of dimensions
    ...
    for word in word_atlas:
        if sum(post_counts_old[word]) > 0:
            for vec in range(n):
                post_counts[word][vec] = (post_counts_old[word][vec])/(sum(post_counts_old[word]))
...
with open('poline3.csv', 'a', encoding='utf-8', newline='') as file:
